#!/usr/bin/env bash
# Download and install the latest irodsfs-pool Linux release.
set -euo pipefail

repository="cyverse/irodsfs-pool"
...
for required_file in "${binary_name}" install.sh config.yaml "${binary_name}.service"; do
