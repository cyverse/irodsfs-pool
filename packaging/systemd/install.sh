#!/usr/bin/env bash
# Install irodsfs-pool as a systemd service. This script works both from a
# source checkout and from the release archive layout.
set -euo pipefail

service_name="irodsfs-pool"
service_user="irodsfs-pool"
install_prefix="/usr/bin"
config_dir="/etc/irodsfs-pool"
...
# The recovery key encrypts persisted session credentials and must remain
...
# The packaged paths are created explicitly because the pool writes its
